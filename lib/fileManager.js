...
class FileManager {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._fs.readdir(path, (error, content) => {
        if (error) {
          ...
          let listing = content.map((c) => this._addEntry(this.join(path, c), entries));
  ...
  listDeep(path, entries = { files: [], dirs: [] }) {
    return this.list(path).then((currEntries) => {
      ...
      if (currEntries.dirs.length > 0) {
        ...
        let listing = currEntries.dirs.map((d) => this.listDeep(d, entries));
   ...
   */
  exists(path) {
    return this.stat(path).then(() => true)
    .catch((error) => {
        ...
        if (exists) {
          ...
          this._fs.open(path, 'w', (error) => {
            if (error && error.code === 'ENOENT') {
              ...
              this.createDir(dirs).then(this.createFile.bind(this, path))
              .then(resolve.bind(null, path));
            }
            else if (error) {
              reject(error);
            }
      ...
      this._fs.rmdir(path, (error) => {
        if (error && error.code === 'ENOTEMPTY') {
          this.list(path).then((entries) => {
            let filesRemoval = entries.files.map(this.removeFile.bind(this));
            return Promise.all(filesRemoval).then(() => entries);
          })
          .then(entries => {
            let dirsRemoval = entries.dirs.map(this.removeDir.bind(this));
            return Promise.all(dirsRemoval);
          })
          .then(() => {
            return this.removeDir(path).then(resolve);
          });
        }
        else if (error) {
          reject(error);
  ...
  _dirSize(path, sizes = []) {
    return this.list(path)
      .then((entries) => {
        ...
        if (files.length > 0) {
          let sum = files.map((f) => this.stat(f).then((s) => sizes.push(s.size)));
          return Promise.all(sum).then(() => dirs);
        }
        return dirs;
      })
      .then((dirs) => {
        if (dirs.length > 0) {
          let sum = dirs.map((d) => this._dirSize(d, sizes));
          return Promise.all(sum).then((sizes) => sizes);
        }
        return sizes;
      })
  ...
  _createDirAtDepth(dirs, depth, resolve) {
    ...
    return this.createDir(path, dirs, depth).then(resolve);
  }
}
